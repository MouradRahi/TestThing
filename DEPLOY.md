...
- **Large image uploads (>4.5 MB) via the admin can fail on Vercel.** Serverless functions cap request bodies at ~4.5 MB, and admin uploads go through the API route. If you hit this, enable browser-direct uploads: set `clientUploads: true` on the `s3Storage` plugin in `payload.config.ts` **and** add a CORS rule on the Supabase bucket allowing `PUT` from your domain. (Ask me to wire this up.) Workaround until then: keep product photos under ~4 MB.
