...
| `WHATSAPP_STATUS_TEMPLATE_NAME` | Name of an **approved** template used to message the *customer* on later order-status changes (confirmed/shipped/delivered/…). Body: `"Hi! Here's the latest update on your trackID.lb order {{1}}. Current status: {{2}}. We'll message you here again as soon as anything changes — thank you for your support!"` ({{1}} = order number, {{2}} = status phrase). Unset = feature off. **Note:** the shorter `"Update on your order {{1}}: {{2}}."` is REJECTED by Meta ("too many variables for its length") — templates need enough non-variable text to review, hence the longer body. Also note {{2}} is a full phrase (e.g. "Your order is on its way"), not a single word — see `STATUS_EMAIL_COPY` in `notifications.ts`. |
