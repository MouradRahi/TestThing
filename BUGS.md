...
### B1 ☐ No forgot-password / reset-password flow — customers get permanently locked out
...
- **Fix**: Payload has `payload.forgotPassword()` / `payload.resetPassword()` built in for auth collections. Add `POST /api/account/forgot-password` (rate-limited, always responds success to avoid email enumeration) + `POST /api/account/reset-password`; a `/account/reset/[token]` page; a branded email via the existing Resend setup in `notifications.ts` (respect `resolveBrandCopy`). Add "Forgot password?" link to the login form. Also add change-password to the account page (needs current-password confirmation).
- Files: `src/app/api/account/*`, `src/components/account/AuthForm.tsx`, `src/app/[locale]/(frontend)/account/*`, `src/lib/notifications.ts`, `src/collections/Customers.ts` (`auth.forgotPassword` token expiry config), `messages/{en,ar}.json`.
